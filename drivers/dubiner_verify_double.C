...
int main(int argc, char** argv)
{
  ...
  for (unsigned dubiner_degree=/*0*/max_dubiner_degree;
       dubiner_degree <= max_dubiner_degree;
       ++dubiner_degree)
    {
      ...
      for (unsigned q=0; q<Nq; ++q)
        {
          ...
          std::vector<Point<double>> current_derivs;

          dubiner.p_double
            (dubiner_degree,
             /*xi=*/  conical_rule_points[q](0).get_d(),
             /*eta=*/ conical_rule_points[q](1).get_d(),
             current_vals,
             current_derivs);

          if (current_vals.size() != Np || current_derivs.size() != Np)
            {
              std::cerr << "current_vals.size() = " << current_vals.size()
                        << " should be Np = " << Np << std::endl;
              std::cerr << "current_derivs.size() = " << current_derivs.size()
                        << " should be Np = " << Np << std::endl;
              std::abort();
            }
          ...
          //   {
          //     std::cout << "current_derivs[" << i << "]="
          //               << current_derivs[i](0) << ", "
          //               << current_derivs[i](1) << std::endl;
          //     // std::cout << "current_vals[" << i << "]="
          //                  << current_vals[i] << std::endl;
          //   }
          ...
          // Accumulate the phi(i)*phi(j) and dphi(i)*dphi(j)
          // integrals using conical product rule quadrature.
          for (unsigned i=0; i<Np; ++i)
            for (unsigned j=0; j<Np; ++j)
              {
                mass_sums(i,j) += conical_rule_weights[q].get_d() * mass(i,j);
                laplace_sums(i,j) += conical_rule_weights[q].get_d() * laplace(i,j);
              }
        }
      ...
      for (unsigned i=0; i<Np; ++i)
        for (unsigned j=0; j<Np; ++j)
          {
            if ((i != j) && (std::abs(mass_sums(i,j)) > 1.e-10))
              {
                std::cerr << "Matrix entry " << i << "," << j
                          << " should be zero, but is "
                          << mass_sums(i,j) << std::endl;
                std::abort();
              }
          }
